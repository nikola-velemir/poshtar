...
#let physical() = context {
  let chapters = query(heading.where(level: 1, outlined: true)).filter(it => not it.numbering == none).len()
  ...
  let appendices = query(heading.where(level: 1))
    .filter(x => x.has("label") and repr(x.label).starts-with("<dodatak"))
    .len()
...
#let hints() = context {
  let enabled = sys.inputs.at("hints-enabled", default: "false") == "true"
  if enabled {
    let elems = query(
      <hint>,
    )
    ...
    for body in elems {
      text([+ #link(body.location(), body)], blue)
    }
  }
}
#let hint(x) = {
